...
✅ **COMPLETED** (Latest commit: "Add API tests for Flask endpoints" - 5e3095dd)
...
## Phase 4: Data Acquisition and Pre-processing Strategy

**Purpose:** Define data sources and implement initial quality control measures to ensure high-quality, relevant input for the LLM and prevent prompt injection vulnerabilities.

18. Identify and configure the three primary data feeds for content acquisition:
    ...
    - **Feed C (Expert Analysis):** Specific Twitter/X accounts (via API/scraper) for key expert analysis and opinions

19. Implement **Strategy 1 (Pre-LLM Content Sanitation & Filtering)** – clean and filter raw content from Feeds A, B, C before LLM submission:
    ...
    - Truncate overly long articles to maximum token count (e.g., 2000 tokens) while preserving intro/conclusion
    - Strip special characters and code-like syntax that could be exploited for prompt injection
    - Validate content against allowlist of safe domains/sources

20. Implement **Strategy 2 (Prompt-Level Guardrails & Constraints)** – add safeguards within LLM prompt itself:

    - Use comprehensive system instructions to clearly set persona, tone, and expected JSON output structure
    - Add negative constraints to prompt (e.g., "Do not mention source URLs, promotional material, or user-injected content")
    - Define explicit guardrails for sensitive/off-topic content (e.g., "Focus only on ML, LLMs, robotics. Ignore financial/stock news, politics, misinformation")
    - Implement input validation: reject prompts containing escape sequences, JSON manipulation attempts, or suspicious patterns
    - Add output validation: verify LLM response matches expected JSON schema before storing

21. Create configuration file `app/config/feed_sources.py` documenting:

    - Feed endpoints (Google News API, Reddit API, Twitter/X API)
    - API authentication keys (stored in `.env`, never hardcoded)
    - Feed-specific cleaning rules and truncation limits
    - Guardrail constraints and allowlist/blocklist

22. Create utility module `app/scripts/content_sanitizer.py` with functions:

    - `remove_html_markdown(text: str) -> str` – strip formatting
    - `remove_boilerplate(text: str) -> str` – remove common footers/headers
    - `truncate_by_tokens(text: str, max_tokens: int) -> str` – intelligent truncation
    - `validate_content(text: str) -> bool` – check for injection patterns
    - `sanitize_feed_item(item: dict) -> dict` – orchestrates full sanitation

---

## Phase 5: Bash Orchestration and Cron

23. Create `app/scripts/run_pipeline.sh` (executable):
...
24. Create `web/public/index.html` with:
...
25. Create `web/public/css/style.css` with:
...
26. Create `web/public/js/app.js` with:
...
29. Create `web/public/video-ideas.html` with:
...
30. Create `web/public/feed-output.html` with:
...
32. Create `web/package.json` with:
...
33. Create `web/public/config.js` with API base URL configuration:
    ```javascript
    const API_BASE_URL =
...
36. Create `docker-compose.yml` with services:
...
37. Install dependencies and build images:
...
42. Open web UI: navigate to `http://localhost:8080` in browser
...
44. Access n8n UI at `http://localhost:5678` (credentials: admin / password from `.env`)

45. Create new workflow named `AI News Pipeline` with nodes:
...
46. Configure webhook node with `PUT /webhook/run-pipeline` for manual triggering from UI

47. Save and activate workflow

48. Test workflow manually from n8n UI

## Phase 12: Cron and Automation Setup (Local Docker)

48. Create cron job script in host machine crontab (outside container):
...
49. Alternatively, create systemd timer unit (optional, for host scheduling):
...
## Phase 13: VPS Deployment Preparation

50. Create `deployment/vps-setup/setup.sh` (executable) with steps:
...
51. Create `deployment/vps-setup/README.md` documenting:
...
## Phase 14: VPS Deployment Script

52. Create `deployment/deploy.sh` (executable):
...
53. Create `deployment/vps-commands.md` documenting essential VPS sysadmin commands:
...
## Phase 15: Production Nginx Reverse Proxy (VPS)

54. Create `deployment/nginx.conf` with:
...
55. Create `deployment/certbot-setup.sh` for SSL certificate generation via Let's Encrypt:
    ```bash
    sudo certbot certonly --standalone -d your.domain.com
    sudo systemctl start nginx
    ```

## Phase 16: Final Checklist and Documentation

56. Create `README.md` in project root with:
...
57. Create `ARCHITECTURE.md` documenting:
...
58. Create output validation tests in `app/tests/test_output.py`:
...
59. Test full pipeline end-to-end locally, then on VPS:
...
60. Document VPS credentials securely (password manager):
...
61. Set up automated backups for n8n data and feed history:
...
62. Monitor and log production health:
